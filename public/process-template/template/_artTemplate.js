...
!(function () {
    ...
    var REMOVE_RE = /\/\*[\w\W]*?\*\/|\/\/[^\n]*\n|\/\/[^\n]*$|"(?:[^"\\]|\\[\w\W])*"|'(?:[^'\\]|\\[\w\W])*'|\s*\.\s*[$\w\.]+/g;
    var SPLIT_RE = /[^\w$]+/g;
    var KEYWORDS_RE = new RegExp(["\\b" + KEYWORDS._replace(/,/g, '\\b|\\b') + "\\b"]._join('|'), 'g');
    var NUMBER_RE = /^\d[^,]*|,\d[^,]*/g;
    var BOUNDARY_RE = /^,+|,+$/g;
    var SPLIT2_RE = /^$|,+/;
    ...
    function getVariable (code) {
        ...
        ._replace(REMOVE_RE, '')
        ._replace(SPLIT_RE, ',')
        ._replace(KEYWORDS_RE, '')
        ._replace(NUMBER_RE, '')
        ._replace(BOUNDARY_RE, '')
        ._split(SPLIT2_RE);
    };
    ...
    function compiler (source, options) {
        ...
        try {
            ...
            var Render = new _Function("$data", "$filename", code);
        ...
        function logic (code) {
            ...
            if (code.indexOf('=') === 0) {
                ...
                code = code._replace(/^=[=#]?|[\s;]*$/g, '');
    
                // 对内容编码
                if (escapeSyntax) {
    
                    var name = code._replace(/\s*\([^\)]+\)/, '');
    ...
    defaults.parser = function (code, options) {
        ...
        // var match = code._match(/([\w\$]*)(\b.*)/);
        // var key = match[1];
        // var args = match[2];
        // var split = args.split(' ');
        // split.shift();
    
        code = code._replace(/^\s/, '');
    
        var split = code.split(' ');
        var key = split.shift();
        var args = split._join(' ');
