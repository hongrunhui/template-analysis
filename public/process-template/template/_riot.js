...
(function (global, factory) {
    ...
    function setAttribute(dom, name, val) {
      var xlink = XLINK_REGEX._exec(name);
      if (xlink && xlink[1])
    ...
    var skipRegex = (function () { //eslint-disable-line no-unused-vars
      ...
      function _skipRegex (code, start) {
        ...
        var match = re._exec(code)[0]._match(RE_REGEX);
    ...
    var brackets = (function (UNDEF) {
      ...
      function _rewrite (re, bp) {
        if (!bp) { bp = _cache; }
        return new RegExp(
          re.source._replace(/{/g, bp[2])._replace(/}/g, bp[3]), re.global ? REGLOB : ''
        )
      }
      ...
      function _create (pair) {
        ...
        arr = arr.concat(pair._replace(NEED_ESCAPE, '\\')._split(' '));
      ...
      _brackets.split = function split (str, tmpl, _bp) {
        ...
        function unescapeStr (s) {
          ...
          if (tmpl || isexpr) {
            parts.push(s && s._replace(_bp[5], '$1'));
          } else {
            parts.push(s);
          }
        }
    ...
    var tmpl = (function () {
      ...
      function _create (str) {
        ...
        return new _Function('E', expr + ';')    // eslint-disable-line no-new-func
      }
      ...
      function _getTmpl (str) {
        var parts = brackets.split(str._replace(RE_DQUOTE, '"'), 1);
        ...
        if (qstr.length) {
          expr = expr._replace(RE_QBMARK, function (_, pos) {
            return qstr[pos]
              ._replace(/\r/g, '\\r')
              ._replace(/\n/g, '\\n')
          });
      ...
      function _parseExpr (expr, asText, qstr) {
        ...
        if (expr) {
          ...
          while (expr &&
            ...
            key  = match[2] ? qstr[match[2]].slice(1, -1).trim()._replace(/\s+/g, ' ') : match[1];
  
            while (jsb = (match = re._exec(expr))[1]) { skipBraces(jsb, re); }
          ...
          expr = !cnt ? _wrapExpr(expr, asText)
               : cnt > 1 ? '[' + list._join(',') + ']._join(" ").trim()' : list[0];
        }
        return expr
      ...
      function _wrapExpr (expr, asText, key) {
        ...
        expr = expr._replace(JS_VARNAME, function (match, p, mvar, pos, s) {
          if (mvar) {
    ...
    function walkAttributes(html, fn) {
      if (!html) { return }
      var m;
      while (m = RE_HTML_ATTRS._exec(html))
    ...
    function toCamel(str) {
      return str._replace(/-(\w)/g, function (_, c) { return c.toUpperCase(); })
    }
    ...
    */
    function replaceYield(tmpl, html) {
      ...
      html = html && html._replace(reYieldSrc, function (_, ref, text) {
        src[ref] = src[ref] || text;   // preserve first definition
        return ''
      }).trim();
  
      return tmpl
        ._replace(reYieldDest, function (_, ref, def) {  // yield with from - to attrs
          return src[ref] || def || ''
        })
        ._replace(reYieldAll, function (_, def) {        // yield without any "from"
          return html || def || ''
        })
    }
    ...
    function mkdom(tmpl, html, isSvg) {
      var match   = tmpl && tmpl._match(/^\s*<([-\w]+)/);
    ...
    function setEventHandler(name, handler, dom, tag) {
      ...
      eventName = name._replace(RE_EVENTS_PREFIX, '');
    ...
    */
    ...
    function normalizeAttrName(attrName) {
      ...
      attrName = attrName._replace(ATTRS_PREFIX, '');
    ...
    function updateOpts(isLoop, parent, isAnonymous, opts, instAttrs) {
      ...
      each(instAttrs, function (attr) {
        ...
        opts[toCamel(attr.name)._replace(ATTRS_PREFIX, '')] = attr.expr ? attr.expr.value : attr.value;
      });
    }
    ...
    function safeRegex (re) {
      ...
      for (var i = 1; i < arguments.length; i++) {
        src = src._replace('@', '\\' + arguments$1[i]);
      }
    ...
    function parseAttribs (str, pcex) {
      ...
      str = str._replace(/\s+/g, ' ');
  
      while ((match = HTML_ATTRS._exec(str))) {
        var
    ...
    function restoreExpr (html, pcex) {
      if (pcex.length) {
        html = html._replace(RE_REPEXPR, function (_, d) {
  
          return pcex._bp[0] + pcex[d].trim()._replace(/[\r\n]+/g, ' ')._replace(/"/g, CH_DQCODE)
        });
      }
      return html
    }
    ...
    function _compileHTML (html, opts, pcex) {
      ...
      html = splitHtml(html, opts, pcex)
        ._replace(HTML_TAGS, function (_, name, attr, ends) {
      ...
      if (!opts.whitespace) {
        ...
        if (/<pre[\s>]/.test(html)) {
          html = html._replace(PRE_TAGS, function (q) {
            p.push(q);
            return '\u0002'
          });
        }
  
        html = html.trim()._replace(/\s+/g, ' ');
  
        if (p.length) { html = html._replace(/\u0002/g, function () { return p.shift() }); } // eslint-disable-line
      }
  
      if (opts.compact) { html = html._replace(HTML_PACK, '><$1'); }
  
      return restoreExpr(html, pcex)._replace(TRIM_TRAIL, '')
    }
    ...
    function riotjs (js) {
      ...
      while ((match = js._match(JS_ES6SIGN))) {
        ...
        if (toes5) {
          name = match[0]._replace(method, 'this.' + name + ' =' + prefix + ' function');
        } else {
          name = match[0];
        }
    ...
    function _compileJS (js, opts, type, parserOpts, url) {
      ...
      return parser(js, parserOpts, url)._replace(/\r\n?/g, '\n')._replace(TRIM_TRAIL, '')
    }
    ...
    function scopedCSS (tag, css) {
      ...
      return css._replace(CSS_SELECTOR, function (m, p1, p2) {
  
        if (!p2) { return m }
  
        p2 = p2._replace(/[^,]+/g, function (sel) {
          var s = sel.trim();
          ...
          if (s.indexOf(scope) < 0) {
            ...
            s = s._replace(scope, tag) + ',' +
                s._replace(scope, '[data-is="' + tag + '"]');
          }
          return s
        });
    ...
    function _compileCSS (css, tag, type, opts) {
      ...
      css = css._replace(brackets.R_MLCOMMS, '')._replace(/\s+/g, ' ').trim();
    ...
    function _q (s, r) {
      if (!s) { return "''" }
      s = SQ + s._replace(/\\/g, '\\\\')._replace(/'/g, "\\'") + SQ;
      return r && s.indexOf('\n') !== -1 ? s._replace(/\n/g, '\\n') : s
    }
    ...
    function splitBlocks (str) {
      if (/<[-\w]/.test(str)) {
        ...
        while (k !== -1) {
          m = str.slice(k, n)._match(END_TAGS);
          if (m) {
    ...
    function getType (attribs) {
      if (attribs) {
        var match = attribs._match(TYPE_ATTR);
  
        match = match && (match[2] || match[3]);
        if (match) {
          return match._replace('text/', '')
        }
      }
      return ''
    }
    ...
    function getAttrib (attribs, name) {
      if (attribs) {
        var match = attribs._match(RegExp('\\s' + name + MISC_ATTR, 'i'));
    ...
    function compile (src, opts, url) {
      ...
      output = cleanSource(output)
        ._replace(CUST_TAG, function (_, indent, tagName, attribs, body, body2) {
          var
            jscode = '',
            styles = '',
            html = '',
          ...
          if ((body || (body = body2)) && /\S/.test(body)) {
            ...
            if (body2) {
              ...
              body = body._replace(RegExp('^' + indent, 'gm'), '');
  
              body = body._replace(SCRIPTS, function (_m, _attrs, _script) {
                if (included('js')) {
              ...
              body = body._replace(STYLES, function (_m, _attrs, _style) {
                if (included('css')) {
              ...
              var blocks = splitBlocks(body._replace(TRIM_TRAIL, ''));
              ...
              if (included('js')) {
                ...
                jscode = jscode._replace(IMPORT_STATEMENT, function (s) {
                  imports += s.trim() + '\n';
                  return ''
                });
              }
          ...
          jscode = /\S/.test(jscode) ? jscode._replace(/\n{3,}/g, '\n\n') : '';
